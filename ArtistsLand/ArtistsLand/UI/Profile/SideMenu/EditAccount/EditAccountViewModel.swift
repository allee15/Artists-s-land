...
class EditAccountViewModel: BaseViewModel {
    ...
    func editInfo() {
        if nickname.isEmpty {
            ...
            self.errorMessageEmail = "This field can't be empty."
        } else {
            userService.editAccount(nickname: userInfo.nickname != nickname ? nickname : nil,
                                    email: userInfo.email != email ? email : nil)
                .sink { _ in
